...
void tournoi(pop* joueurs, double (*f) (ind*, ind*)) { // la taille doit être une puissance de 2 impaire et pas d'égalité
...
pop* creer_pop(int nb_joueurs, int nb_mutateurs) {
    ...
    for (int i = capacite / 2; i != capacite; i += 1) {
        individus[i] = creer_ind_rand();
    }
...
void reproduction(pop* joueurs) {
    ...
    for (int i = 0; i != nb; i += 1) {
        for (int j = 0; j != nb_enfants; j += 1) {
            joueurs->individus[nb_enfants * i + j + nb2] = copy_ind(joueurs->individus[nb_enfants * i + j]);
            muter_ind(joueurs->individus[nb_enfants * i + j + nb2], joueurs->mutateurs->individus[i]);
            joueurs->mutateurs->individus[i]->enfants[j] = joueurs->individus[nb_enfants * i + j + nb2];
        }
    }
}
...
void evolution(pop* joueurs, double (*f) (ind*, ind*)) {
    tournoi(joueurs, f);
    tournoi2(joueurs->mutateurs);
    selection2(joueurs->mutateurs, joueurs->log2_capacite);
    selection(joueurs);
    reproduction2(joueurs->mutateurs);
    reproduction(joueurs);
}

void diviser_pop2(pop2* mutateurs, int nb_individus) {
    int nb_enfants = mutateurs->individus[0]->nb_enfants * 2;
    selection2(mutateurs, nb_individus);
    for (int i = mutateurs->capacite / 4; i != mutateurs->capacite / 2; i += 1) {
        lib_mem_mut(mutateurs->individus[i]);
...
void diviser_pop(pop* joueurs) {
    ...
    for (int i = joueurs->mutateurs->capacite / 4; i != joueurs->mutateurs->capacite / 2; i += 1) {
        lib_mem_mut(joueurs->mutateurs->individus[i]);
...
void evolutions(pops* populations, double (*f) (ind*, ind*)) {
    ...
    for (int i = 0; i != 9; i += 3) {
        if (premier == i || premier == i + 1 || premier == i + 2) {
            ...
            if (populations->victoires[i / 3] >= 5) {
            ...
            if (premier1 == i) {
                ...
                if (populations->individus[i]->nb_vic >= 5) {
                    if (i == 0 && populations->individus[0]->mutateurs->log2_capacite > 2) {
                        for (int j = 0; j != 3; j += 1) {
                            diviser_pop2(populations->individus[j]->mutateurs, populations->individus[j]->log2_capacite);
                        }
                        pass1 = false;
    ...
    if (pass) {
        if (pass1) {
            for (int i = 0; i != 3; i += 1) {
                selection2(populations->individus[i]->mutateurs, populations->individus[i]->log2_capacite);
    ...
    for (int i = 0; i != 9; i += 1) {
        if (populations->individus[i]->individus[0] != populations->individus[premier]->individus[0]) {
            lib_mem_ind(populations->individus[i]->individus[populations->individus[i]->capacite / 2 - 1]);
            populations->individus[i]->individus[populations->individus[i]->capacite / 2 - 1] = copy_ind(populations->individus[premier]->individus[0]);
        }
    }
    free(mem);
...
void afficher_joueur(ind* joueur) {
    ...
    printf("perf : %.15f\n", g2(joueur)); // changer g2 en g1 si on utilise f1
}
...
void afficher_couple(ind* joueur, mut* mutateur) {
    ...
    printf("perf : %.15f  score : %d\n", g2(joueur), mutateur->score); // changer g2 en g1 si on utilise f1
}
...
int main() {
    srand(time(NULL));
    //ind* joueur = main2(f1, 3, 7, 4);
    ind* joueur = main2(f2, 20, 8, 4);
