...
pop2* creer_pop2(int capacite, int nb_enfants) {
...
void selection2(pop2* mutateurs, int nb_individus) {
    ...
    int c = 0;
    for (int i = nb_individus; i != -nb_individus - 1; i -= 1) {
...
void reproduction(pop* joueurs, pop2* mutateurs) {
    ...
    for (int i = 0; i != nb; i += 1) {
        for (int j = 0; j != nb_enfants; j += 1) {
            ...
            muter_ind(joueurs->individus[nb_enfants * i + j + nb2], mutateurs->individus[i]);
            mutateurs->individus[i]->enfants[j] = joueurs->individus[nb_enfants * i + j + nb2];
        }
    }
}
...
void evolution(pop* joueurs, pop2* mutateurs, int nb_joueurs, double (*f) (ind*, ind*)) {
    tournoi(joueurs, nb_joueurs, f);
    tournoi2(mutateurs);
    selection2(mutateurs, nb_joueurs);
    selection(joueurs);
...
void afficher_joueur(ind* joueur) {
    ...
    printf("perf : %.10f\n", g2(joueur)); // changer g1 en g2 si on utilise f2
}
...
void afficher_couple(ind* joueur, mut* mutateur) {
    ...
    printf("perf : %.10f  score : %d\n", g2(joueur), mutateur->score); // changer g1 en g2 si on utilise f2
}

ind* main2(double (*f) (ind*, ind*), int temps, int nb_joueurs, int nb_mutateurs) {
    int t0 = time(NULL);
    pop* joueurs = creer_pop(1 << nb_joueurs);
    pop2* mutateurs = creer_pop2(1 << nb_mutateurs, 1 << (nb_joueurs - nb_mutateurs - 1));
    int nb_enfants = mutateurs->individus[0]->nb_enfants;
    for (int i = 0; i != mutateurs->capacite; i += 1) {
        for (int j = 0; j != nb_enfants; j += 1) {
            mutateurs->individus[i]->enfants[j] = joueurs->individus[nb_enfants * i + j];
        }
    }
    int i = 0;
    int nb_genrations = 0;
    while(time(NULL) - t0 < temps) {
        evolution(joueurs, mutateurs, nb_joueurs, f);
...
int main() {
    srand(time(NULL));
    //ind* joueur = main2(f1, 1, 7, 4);
    ind* joueur = main2(f2, 10, 12, 4);
